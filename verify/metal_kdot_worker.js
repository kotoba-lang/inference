...
async function project(req,input) {
  ...
  const q8=gpuBuffer(quantizeQ8(input,req.positions,req.cols),SU);
  const output=device.createBuffer({size:req.rows*req.positions*4,usage:SU});
  const params=gpuBuffer(new Uint8Array(new Uint32Array([req.rows,req.cols,req.type,req.positions]).buffer),UU);
  ...
  const staging=device.createBuffer({size:req.rows*req.positions*4,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});
  const enc=device.createCommandEncoder(),pass=enc.beginComputePass();pass.setPipeline(pipeline);pass.setBindGroup(0,bg);pass.dispatchWorkgroups(Math.ceil(req.rows/64),req.positions,1);pass.end();enc.copyBufferToBuffer(output,0,staging,0,req.rows*req.positions*4);device.queue.submit([enc.finish()]);
  await staging.mapAsync(GPUMapMode.READ);const result=new Float32Array(staging.getMappedRange().slice(0));staging.unmap();params.destroy();q8.destroy();output.destroy();staging.destroy();return result;
}
...
for(;;){
  const h=await readExact(52);if(h===null)break;
  try{const v=new DataView(h.buffer);if(v.getUint32(0,false)!==0x4b444f54)throw new Error("bad protocol magic");const req={type:v.getInt32(4,false),rows:Number(v.getBigInt64(8,false)),cols:Number(v.getBigInt64(16,false)),positions:v.getInt32(24,false),offset:Number(v.getBigInt64(28,false)),tensorBytes:Number(v.getBigInt64(36,false)),pathLen:v.getInt32(44,false),keyLen:v.getInt32(48,false)};req.path=decoder.decode(await readExact(req.pathLen));req.key=decoder.decode(await readExact(req.keyLen));const raw=await readExact(req.positions*req.cols*4),rv=new DataView(raw.buffer),input=new Float32Array(req.positions*req.cols);for(let i=0;i<input.length;i++)input[i]=rv.getFloat32(i*4,false);await respond(0,await project(req,input));}catch(e){await respond(-1,e?.stack??e);}
}
