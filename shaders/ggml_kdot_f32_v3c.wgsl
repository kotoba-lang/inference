// ggml K-quant dot, f32 activations, sub-block per thread, IQ4_XS byte -> value-pair table (v3c, iteration 4).
//
...
var<workgroup> kvtab: array<f32, 16>;   // IQ4_XS codebook staged once per workgroup
var<workgroup> kvpair: array<vec2<f32>, 256>;   // byte -> (low-nibble value, high-nibble value)
...
fn subblock_dot(tt: u32, wb: u32, j: u32) -> f32 {
  ...
  if tt == 23u {
    ...
    for (var w = 0u; w < 4u; w++) {
      ...
      let base = j * 32u + w * 4u;
      let p0 = kvpair[word & 255u]; let p1 = kvpair[(word >> 8u) & 255u]; let p2 = kvpair[(word >> 16u) & 255u]; let p3 = kvpair[word >> 24u];
      acc += p0.x * xs[base] + p0.y * xs[base + 16u] + p1.x * xs[base + 1u] + p1.y * xs[base + 17u]
           + p2.x * xs[base + 2u] + p2.y * xs[base + 18u] + p3.x * xs[base + 3u] + p3.y * xs[base + 19u];
    }
...
fn main(@builtin(workgroup_id) wg: vec3<u32>, @builtin(local_invocation_id) lid: vec3<u32>) {
  ...
  var acc = 0.0;
  if t < 16u { kvtab[t] = f32(kv_lookup(t)); }
  for (var e = t; e < 256u; e += 64u) { kvpair[e] = vec2<f32>(f32(kv_lookup(e & 15u)), f32(kv_lookup(e >> 4u))); }
