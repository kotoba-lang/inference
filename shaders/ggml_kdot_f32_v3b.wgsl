...
fn subblock_dot(tt: u32, wb: u32, j: u32) -> f32 {
  ...
  if tt == 23u {
    ...
    for (var w = 0u; w < 4u; w++) {
      ...
      let base = j * 32u + w * 4u;
      acc += kvtab[word & 15u] * xs[base] + kvtab[(word >> 4u) & 15u] * xs[base + 16u]
           + kvtab[(word >> 8u) & 15u] * xs[base + 1u] + kvtab[(word >> 12u) & 15u] * xs[base + 17u]
           + kvtab[(word >> 16u) & 15u] * xs[base + 2u] + kvtab[(word >> 20u) & 15u] * xs[base + 18u]
           + kvtab[(word >> 24u) & 15u] * xs[base + 3u] + kvtab[(word >> 28u) & 15u] * xs[base + 19u];
    }
